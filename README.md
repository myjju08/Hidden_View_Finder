...
- 순위를 확정한 뒤 설명과 이미지 프롬프트를 만듭니다. 기본 가상 Top 3의 **AI 분위기 참고 이미지 3장**을 포함하며, 다른 입력에는 `not_generated` 상태와 프롬프트를 반환합니다. API 키가 없으면 실행 중 이미지를 생성하지 않습니다.
...
실시간 생성은 `.env`에 `OPENAI_API_KEY`를 설정하고 서버를 재시작하면 사용할 수 있습니다. 결과 카드의 **예상 풍경 생성**을 누르면 보이는 대상의 명칭·거리·높이·방향·시야각을 반영한 이미지가 표시됩니다. 키 외 필수 이미지 설정이나 추가 패키지는 없습니다. [설정·output JSON 변환·Python 모듈 사용법](docs/scene-image.md).
