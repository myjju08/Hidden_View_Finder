...
Benchmarks distinguish creation/inspection, process launch/import, opening, warm **uncached** viewsheds, exact cache hits, read/validation, native computation, masking, sparse reference checks and export. They use spatially distributed targets and 30 timed runs per radius, with RSS and disk measurements; targets lacking required coverage are explicitly skipped. Filesystem caches are uncontrolled and never called cold disk; no privileged flushing or JIT is used. The 5 km/5 m design goals are measured goals, not a promise on other machines or terrain. Real Seoul runs and limitations are documented in [the real-data benchmark guide](docs/benchmarks.md). A separately prepared local 2 m Seoul product is unavailable. Report paths written as code instead of links are local generated artifacts excluded from publication.
